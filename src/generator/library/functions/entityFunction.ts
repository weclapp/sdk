...
export const entityFunction = ({path, methods}: EndpointPath, target: Target): Functions => {
        ...
        if (TOP_ID_REGEXP.test(path.path)) {
            functions.add(target, {
                ...
                code: {
                    ...
                    signature: `unique<Query extends EntityQuery<${returnType}, ${relatedEntities}>>(id: string, options?: Query)`,
                    returnType: `UniqueReturn<${returnType}, ${relatedEntities}, Query>`,
                    returnValue: `_unique<${returnType}, ${relatedEntities}, Query>('/${path.entity}', id, options)`
                }
            });
            ...
            functions.add(target, {
                code: {
                    ...
                    signature: `${buildSpecialFunction(path)}<Query extends ${serializedParameters ?? 'Record<string, unknown>'}>(id: string, options${serializedParameters ? '' : '?'}: Query)`,
            ...
            functions.add(target, {
                code: {
                    ...
                    signature: `${buildSpecialFunction(path)}<Query extends ${serializedParameters ?? 'Record<string, unknown>'}>(options${serializedParameters ? '' : '?'}: Query)`,
    ...
    if (methods.post) {
        ...
        const serializedParameters = serializeParameters(resolveParameters(methods.post));
        ...
        if (ID_SPECIAL_REGEXP.test(path.path)) {
            functions.add(target, {
                code: {
                    ...
                    example: `await sdk.${camelCase(entityName)}.${buildSpecialFunction(path)}('9662', {...});`,
                    signature: `${buildSpecialFunction(path)}<Query extends ${serializedParameters ?? 'Record<string, unknown>'}>(id: string, data: ${bodyType}, options${serializedParameters ? '' : '?'}: Query)`,
            ...
            functions.add(target, {
                code: {
                    ...
                    example: `await sdk.${camelCase(entityName)}.${buildSpecialFunction(path)}({...});`,
                    signature: `${buildSpecialFunction(path)}<Query extends ${serializedParameters ?? 'Record<string, unknown>'}>(data: ${bodyType}, options${serializedParameters ? '' : '?'}: Query)`,
