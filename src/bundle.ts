...
const resolveGlobals = (...globals: string[]) =>
  Object.fromEntries(globals.map((v) => [v, "*"]));
...
export const bundle = async (workingDirectory: string, target: Target) => {
  const dist = (...paths: string[]) =>
    resolve(workingDirectory, "dist", ...paths);
  const src = (...paths: string[]) =>
    resolve(workingDirectory, "src", ...paths);
